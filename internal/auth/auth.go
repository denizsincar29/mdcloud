// Package auth — пароли и токены.
//
// Токен — это случайные 32 байта в base64url. В базе лежит только его
// sha256: дамп БД не даёт войти. Кука не используется вообще — ни своей, ни
// чужой: два сайта (облако и редактор) не делят cookie, они обмениваются
// токеном через одноразовый код в URL-фрагменте.
package auth
...
// NewCode выдаёт короткий одноразовый код перехода облако → редактор.
// Он живёт меньше двух минут и сгорает при первом использовании, поэтому
// 16 байт здесь с большим запасом.
